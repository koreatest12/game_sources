...
브라우저의 **생성 후 즉시 다운로드** 버튼은 같은 PUT API로 저장한 직후 `/files/{name}`을 인증 요청해 다운로드합니다. 따라서 생성과 다운로드 모두 서버의 동일한 파일명 검증, Bearer 인증, SHA-256/Range 다운로드 체계를 사용합니다.
